...
export async function analyzeBatchQuality(
  jobIds: string[],
  videoPaths: string[],
  prompts: any[]
): Promise<QualityAnalysis[]> {
  ...
  for (let i = 0; i < jobIds.length; i++) {
    try {
      const analysis = await analyzeVideoQuality(jobIds[i], videoPaths[i], prompts[i]);
      results.push(analysis);
    } catch (error) {
      logger.error({ error: error.message, jobId: jobIds[i] }, 'Batch quality analysis failed for job');
      
      // Add default analysis for failed job
      results.push({
        score: {
          overall: 0.5,
          visual: 0.5,
...
export function getQualityThresholds(): {
  ...
  return {
    minScore: parseFloat(process.env.QUALITY_MIN_SCORE || '0.62'),
    visualThreshold: 0.5,
    audioThreshold: 0.3,
...
export function isQualityPassing(score: QualityScore): boolean {
  const thresholds = getQualityThresholds();
  
  return (
    score.overall >= thresholds.minScore &&
    score.visual >= thresholds.visualThreshold &&
    score.audio >= thresholds.audioThreshold &&
    score.coherence >= thresholds.coherenceThreshold
  );
}
