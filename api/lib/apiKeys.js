...
export function generateApiKey() {
  ...
  const key = 'veo_' + randomBytes.toString('base64url');
  return key;
}
...
export async function getApiKeys(organizationId, userId) {
  try {
    const { data, error } = await supabase
      .from('veo_api_keys')
...
export async function updateApiKey(organizationId, keyId, updateData) {
  try {
...
export async function authenticateApiKey(apiKey) {
  try {
    if (!apiKey || !apiKey.startsWith('veo_')) {
...
export async function checkRateLimit(apiKeyId, endpoint, method) {
  try {
    const now = new Date();
...
export async function recordApiUsage(apiKeyId, endpoint, method, statusCode, responseTime, userAgent, ipAddress) {
  try {
    const { error } = await supabase
      .from('veo_api_key_usage')
      .insert({
...
export function checkPermission(auth, permission) {
...
export function generateJwtToken(auth) {
  const payload = {
    type: 'api_key',
